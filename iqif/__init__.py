...
if platform.system() == "Windows":
    ...
    libiqPath = buildFolder / "libiq-network.so.0.2"
    libizPath = buildFolder / "libiz-network.so.0.2"
    liblifPath = buildFolder / "liblif-network.so.0.2"

try:
    libiq = ctypes.CDLL(libiqPath)
    #libiq = ctypes.CDLL(find_library("iq-network"))
except OSError:
